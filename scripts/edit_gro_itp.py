...
def edit_gro_itp(XL, gro_filename, itp_filename, loop_x):
    ...
    with open('../data/inputs.txt') as f:
        ...
        for i in range(4, len(lis)):
            lis[i] = lis[i].split()        
    with open(XL) as f:
        filelist = f.readlines() # read the reacting pairs file
    ...
    for i in range(length):
        ...
        for j in range(i+1,length):
            line2 = filelist[j].split()
            if line1[1] == line2[1] or line1[3] == line2[3]:
                delete.add(j) # set of repetitious beads which should be deleted
    delete = list(delete)
    delete.sort()
    delete.reverse()
    for n in range(len(delete)):
        filelist.pop(delete[n]) # delete repetitious beads from reacting pairs list
    for m in range(len(filelist)):
        items.append((filelist[m].split()[1],filelist[m].split()[3],\
                    filelist[m].split()[0],filelist[m].split()[2])) # add reacting pairs to a list
        itemslist.append(filelist[m].split()[1]) # add reacting beads to a list
        itemslist.append(filelist[m].split()[3]) # add reacting beads to a list
    ...
    try:
        ...
        for x in range(len(ex_angs)):
            ...
            if (angle_line != 0 and ex_angs[x] != []):
                for y in range(angle_line+1,insert_point):
    ...
    with open('../data/new_angles.txt') as f:
            ...
            try:   # write undefined angles into a new file for user 
                ...
                with open('../undefined_angles.txt', 'w') as g:
                    g.write('%7s (%7s) %7s (%7s) %7s (%7s) \n' %(bonds_found[l][0], one, \
                                            bonds_found[l][1], two, temp, three))
            else:
    ...
    try:
        ...
        for x in range(len(ex_dihs)):
            ...
            if (dihedral_line != 0 and ex_dihs[x] != []):
                for y in range(dihedral_line+1,insert_point):
    ...
    try:
        with open('../data/new_dihedrals.txt') as f:
            f.readlines()
    except FileNotFoundError or NameError:
        pass
    else:
